    ...
    try {
        ...
        itemsSnapshot.forEach((doc) => {
            ...
            if (startDate <= today) {
                ...
                // --- Notification Logic ---
                // Send notification if the new count is less than or equal to the notification threshold
                const notificationThreshold = item.notificationThreshold || 0;
                if (newCount <= notificationThreshold ) {
                    const groupRef = doc.ref.parent.parent;
                    if (groupRef) {
                        const notificationPromise = groupRef.get().then(async (groupDoc) => {
                            const groupData = groupDoc.data();
                            if (groupData && groupData.members) {
                                await sendLowStockNotification(groupData.members, item.name, newCount, notificationThreshold);
                            }
                        });
 ...
 * @param {number} currentCount - The current count of the item.
 * @param {number} threshold - The notification threshold for the item.
 */
async function sendLowStockNotification(memberUids: string[], itemName: string, currentCount: number, threshold: number) {
    ...
    if (tokens.length > 0) {
        ...
        const message: admin.messaging.MulticastMessage = {
            notification: {
                ...
                body: `"${itemName}" is running low! Current: ${currentCount}, Threshold: ${threshold}. Restock Soon!`,
            },
            tokens: tokens,
        };
        ...
        try {
            ...
            console.log(`Successfully sent low stock notification for "${itemName}" (${currentCount}/${threshold}) to ${response.successCount} devices.`);
        } catch (error) {
